...
export default function App() {
  ...
  async function createAccount(newAcc) {
    setLoading(true);
    try {
      if (newAcc.secretPhrase && encPass) {
        newAcc.secretPhraseEncrypted = await encryptString(newAcc.secretPhrase, encPass);
        delete newAcc.secretPhrase;
      }
  ...
  async function updateAccount(id, patch) {
    setLoading(true);
    try {
      if (patch.secretPhrase && encPass) {
        patch.secretPhraseEncrypted = await encryptString(patch.secretPhrase, encPass);
        delete patch.secretPhrase;
      }
  ...
  async function deleteAccount(id) {
    ...
    setLoading(true);
    try {
      await apiFetch('deleteAccount', { id });
      await fetchAll();
    } catch (e) {
      console.error(e);
      alert('Delete failed');
  ...
  async function updateAirdrop(id, patch) {
    setLoading(true);
    try {
      await apiFetch('updateAirdrop', { id, patch });
      await fetchAll();
    } catch (e) {
      console.error(e);
      alert('Update failed');
  ...
  async function assignAccountsToAirdrop(airdropId, accountIds) {
    setLoading(true);
    try {
      await apiFetch('assignAccounts', { airdropId, accountIds });
      await fetchAll();
    } catch (e) {
      console.error(e);
      alert('Assign failed');
  ...
  async function updateAssignment(assignId, patch) {
    setLoading(true);
    try {
      await apiFetch('updateAssignment', { assignmentId: assignId, patch });
      await fetchAll();
    } catch (e) {
      console.error(e);
      alert('Update assignment failed');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function revealSecret(a) {
    ...
    try {
      const dec = await decryptString(a.secretPhraseEncrypted, encPass);
      alert('Secret: ' + dec);
    } catch (e) {
      console.error(e);
  ...
  const totals = useMemo(() => {
    ...
    assignments.forEach(a => {
      earned += Number(a.rewardAmount || 0);
      invested += Number(a.investmentAmount || 0);
    });
    ...
    <div className='min-h-screen p-6'>
      <div className='max-w-6xl mx-auto'>
        <header className='flex justify-between items-center mb-6'>
          <div>
            <h1 className='text-2xl font-bold'>Kesug</h1>
          ...
          <section className='col-span-2 bg-gray-800 p-4 rounded'>
            <div className='flex justify-between items-center mb-4'>
              ...
              <div className='text-sm text-gray-400'>
                Earned: ${totals.earned} • Invested: ${totals.invested}
              </div>
            </div>
            <AirdropsPanel
              airdrops={airdrops}
              ...
              accounts={accounts}
              onCreateAirdrop={createAirdrop}
              onAssign={assignAccountsToAirdrop}
              onUpdateAssignment={updateAssignment}
              isEditor={isEditor}
              onUpdateAirdrop={updateAirdrop}
            />
          </section>
        </main>
      </div>
...
function AccountsPanel({ accounts, onCreate, onUpdate, onDelete, onReveal, isEditor }) {
  ...
  function reset() {
    setForm({ name: '', email: '', twitter: '', discord: '', telegram: '', secretPhrase: '' });
  }
  return (
    <div>
      {isEditor && (
        <div className='mb-3 space-y-2'>
          ...
          <div className='flex gap-2'>
            <button className='px-3 py-1 bg-indigo-600 rounded' onClick={() => { onCreate(form); reset(); }}>Create</button>
            <button className='px-3 py-1 bg-gray-700 rounded' onClick={reset}>Reset</button>
          </div>
        </div>
      )}
        ...
        {accounts.map(a => (
          <div key={a.id} className='p-2 border-b border-gray-700 flex justify-between items-center'>
            <div>
              ...
              <div className='text-xs text-gray-400'>{a.email} • {a.twitter}</div>
            </div>
            <div className='flex gap-2'>
...
// --- Airdrops Panel (full code provided in previous answer)
function AirdropsPanel({ airdrops, assignments, accounts, onCreateAirdrop, onAssign, onUpdateAssignment, isEditor, onUpdateAirdrop }) {
  ...
  return (
    <div>
      ...
      {/* The main Airdrop + Assignments UI */}
                ...
                <div className='flex gap-2'>
                  ...
                  {isEditor && (
                    <button className='px-2 py-1 bg-gray-700 rounded' onClick={async () => {
                      if (!confirm('Mark this airdrop as ENDED?')) return;
        ...
        <div>
          {!selected ? <div className='text-gray-400'>Open an airdrop to manage assignments.</div> : (
            <div>
              ...
              <div className='mb-2'>
                <label className='block text-sm mb-1'>Add accounts</label>
                ...
                <button className='px-3 py-1 mt-2 bg-indigo-600 rounded' onClick={() => { onAssign(selected.id, selection); setSelection([]); }}>Assign Selected</button>
              </div>
